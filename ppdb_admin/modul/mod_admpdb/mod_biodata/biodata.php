...
if (isset($submit) == 'Cari'){
	 ...
	 $res = mysql_query ("select a.adm_id as adm_id, 
						  ...
						  $arg ORDER BY a.nisn ASC LIMIT $posisi,$batas");  // menampilkan data siswa dan status seleksi
...
<form action='' method="GET" name="tabel" id='tabel'>
	...
	<thead>
		<tr>
		  ...
		  <th class="tbl-header ">Verifikasi</th> <!-- sini -->
...
while($items=mysql_fetch_array($res)){
	...
	$verif		=	$items['sts_verifikasi']; // keterangan data sudah dicek/verifikasi belum
	$sel		=	$items['sts_seleksi']; // keterangan seleksi apapakah di lolos/tidak
	$status		= 	$items['sts_bio']; // Keterangan status biografi apa sudah lengkap (otomatis)
	$i++;

	if  ($items['sts_verifikasi']==0){
		if  ($status == 0){
			$ver = "<font color=red>Belum</font>";
		}
		else{
			$ver = "<a href='?module=biodata&act=v_edit&v=$verif&id=$adm_id'>Belum</a>";
		}
	}
	else{
		$ver = "<a href='?module=biodata&act=v_edit&v=$verif&id=$adm_id'>Sudah</a>";
	}
