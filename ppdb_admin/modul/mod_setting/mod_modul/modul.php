...
if (isset($submit) == 'Cari'){
	 ...
	 $res = mysql_query ("select * from ppdb_menu $arg ORDER BY main_id ASC LIMIT $posisi,$batas");		  
	 $jmldata = mysql_num_rows(mysql_query("select * from ppdb_menu $arg"));
}
else{
	 $page		= new Paging;
	 $batas 	= 5;
	 $posisi	= $page->cariPosisi($batas);
	 $res = mysql_query ("SELECT*FROM ppdb_menu ORDER BY main_id LIMIT $posisi,$batas");
	 $jmldata = mysql_num_rows(mysql_query("SELECT*FROM ppdb_menu "));
}
?>
